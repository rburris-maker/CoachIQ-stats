...
function TryoutsView({tryouts, setTryouts, roster, setRoster, teams, activeTeamId, onSwitchTeam, addPlayerToTeam}){
  ...
  const [newEntryVals,  setNewEntryVals]  = useState({}); // {statId: value}
  ...
  const [newStatTimeFormat,setNewStatTimeFormat] = useState("none"); // "none"|"mmss"|"seconds"
  ...
  // ── Calculate improvement: first → latest ────────────────────────────────
  function getImprovement(entries, stat){
    if(!entries||entries.length<2) return null;
    const first = entries[0].value, latest = entries[entries.length-1].value;
    if(isTimeStat(stat)){
      const diff = timeToSecs(first)-timeToSecs(latest); // positive = faster
      if(!diff) return null;
      const abs=Math.abs(diff), mins=Math.floor(abs/60), secs=abs%60;
      const label=mins>0?`${mins}:${String(secs).padStart(2,"0")}`:`${secs}s`;
      return {improved:diff>0, label:`${diff>0?"↑":"↓"} ${label}`};
    } else {
      const fv=parseFloat(first), lv=parseFloat(latest);
      if(isNaN(fv)||isNaN(lv)||fv===lv) return null;
      const diff=isSecondsStat(stat)?fv-lv:lv-fv; // positive = better
  ...
  // ── Add a new measurement entry for a candidate ─────────────────────────
  function addStatEntry(candId, statId, value){
    ...
    upd(t=>({candidates:t.candidates.map(c=>{
      ...
      return {...c,
        customStatHistory:{...history,[statId]:allEntries},
        customStats:{...c.customStats,[statId]:bestVal},
      };
    })}));
  ...
  function removeStatEntry(candId, statId, entryId){
    ...
    upd(t=>({candidates:t.candidates.map(c=>{
      ...
      return {...c,
        customStatHistory:{...history,[statId]:remaining},
        customStats:{...c.customStats,[statId]:bestVal},
      };
    })}));
  }
  ...
  function addCustomStat(){
    ...
    const stat={id:`st${Date.now()}`,label:newStatLabel.trim(),unit:newStatUnit.trim(),
      isTime:newStatTimeFormat==="mmss",timeFormat:newStatTimeFormat};
    upd(t=>({
  ...
  function BulkEntryModal(){
      ...
      <div style={{position:"fixed",inset:0,background:"#000000cc",zIndex:2000,
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:28,maxWidth:400,width:"100%"}}>
          ...
          <button onClick={()=>setShowBulk(false)}
            style={{width:"100%",padding:"11px",background:C.surface,border:`1px solid ${C.border}`,borderRadius:9,color:C.muted,cursor:"pointer",fontSize:14}}>Close</button>
        </div>
      </div>
    );
    ...
    const [local, setLocal] = useState(()=>{
      const init={};
      allSorted.forEach(c=>{ init[c.id]={}; customStats.forEach(s=>{ init[c.id][s.id]=""; }); });
      return init;
    });
    function updateLocal(cid,sid,val){ setLocal(p=>({...p,[cid]:{...p[cid],[sid]:val}})); }

    function flushAndClose(){
      // Each filled cell adds a new history entry
      allSorted.forEach(c=>{
        customStats.forEach(stat=>{
          ...
          if(val&&val.toString().trim()){
            ...
            upd(t=>({candidates:t.candidates.map(cc=>{
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:20,
          ...
          <div style={{padding:"18px 22px 14px",borderBottom:`1px solid ${C.border}`,
            ...
            <div>
              ...
              <div style={{color:C.muted,fontSize:12,marginTop:2}}>Each filled cell adds a new dated entry to that candidate's history</div>
            </div>
            <div style={{color:C.muted,fontSize:11,textAlign:"right",lineHeight:1.8}}>
              <div>Tab → next cell</div><div>Enter ↓ next row</div>
            </div>
          </div>
              ...
              <thead>
                <tr style={{borderBottom:`2px solid ${C.border}`}}>
                  <th style={{padding:"10px 16px",textAlign:"left",color:C.muted,fontWeight:600,
                    fontSize:11,letterSpacing:1,position:"sticky",left:0,background:C.card,minWidth:160,zIndex:2}}>CANDIDATE</th>
                  ...
                  return(
                    <tr key={c.id} style={{borderBottom:`1px solid ${C.border}`,background:ri%2===0?C.card:C.surface}}>
                      <td style={{padding:"8px 16px",position:"sticky",left:0,background:ri%2===0?C.card:C.surface,zIndex:1}}>
                        <div style={{display:"flex",alignItems:"center",gap:8}}>
                          <div style={{width:26,height:26,borderRadius:6,flexShrink:0,background:pc+"22",border:`1.5px solid ${pc}44`,
                      ...
                      {customStats.map(stat=>{
                        ...
                        const val=(local[c.id]||{})[stat.id]||"";
                        const bestSoFar=(c.customStats||{})[stat.id];
                        if(isTimeStat(stat)) return(
                          <td key={stat.id} style={{padding:"5px 8px",textAlign:"center"}}>
                            {bestSoFar&&<div style={{color:C.muted,fontSize:9,marginBottom:2}}>best: {bestSoFar}</div>}
                            <div onKeyDown={e=>handleKeyNav(e,c.id,stat.id)}>
                              <TimeInput value={val} onChange={v=>updateLocal(c.id,stat.id,v)} placeholder="0:00"
                                style={{...cell,ref:el=>{ inputRefs.current[key]=el; }}}/>
                            </div>
                          </td>
                        );
                        if(isSecondsStat(stat)) return(
                          <td key={stat.id} style={{padding:"5px 8px",textAlign:"center"}}>
                            {bestSoFar&&<div style={{color:C.muted,fontSize:9,marginBottom:2}}>best: {bestSoFar}</div>}
                            <input ref={el=>{ inputRefs.current[key]=el; }} type="number" step="0.01"
                              value={val} onChange={e=>updateLocal(c.id,stat.id,e.target.value)}
                              onKeyDown={e=>handleKeyNav(e,c.id,stat.id)} placeholder="4.97" style={cell}/>
                          </td>
                        );
                        return(
                          <td key={stat.id} style={{padding:"5px 8px",textAlign:"center"}}>
                            {bestSoFar&&<div style={{color:C.muted,fontSize:9,marginBottom:2}}>best: {bestSoFar}</div>}
                            <input ref={el=>{ inputRefs.current[key]=el; }} type="number" step="any"
                              value={val} onChange={e=>updateLocal(c.id,stat.id,e.target.value)}
                              onKeyDown={e=>handleKeyNav(e,c.id,stat.id)} placeholder="—" style={cell}/>
                          </td>
                        );
                      })}
                    </tr>
          ...
          <div style={{padding:"14px 22px",borderTop:`1px solid ${C.border}`,display:"flex",gap:10,justifyContent:"flex-end",flexShrink:0}}>
            <button onClick={()=>setShowBulk(false)}
              style={{padding:"10px 20px",background:C.surface,border:`1px solid ${C.border}`,borderRadius:9,color:C.muted,cursor:"pointer",fontSize:13}}>Cancel</button>
            <button onClick={flushAndClose}
              style={{padding:"10px 28px",background:C.accent,border:"none",borderRadius:9,
                color:"#000",fontWeight:800,fontSize:14,cursor:"pointer",fontFamily:"'Oswald',sans-serif"}}>Save All →</button>
          </div>
        </div>
      </div>
    );
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          {customStats.length>0&&(
            <button onClick={()=>setShowBulk(true)}
              style={{display:"flex",alignItems:"center",gap:7,padding:"9px 16px",background:C.accent,
                border:"none",borderRadius:9,color:"#000",fontWeight:800,fontSize:13,cursor:"pointer",fontFamily:"'Oswald',sans-serif"}}>
              ⚡ Bulk Enter
            </button>
          )}
            ...
            {POS_FILTERS.map(pf=>{
              ...
              return(
                <button key={pf} onClick={()=>{setPosFilter(pf);setSelCand(null);}}
                  ...
                  {pf}
                  <span style={{background:posFilter===pf?col+"33":C.border+"66",color:posFilter===pf?col:C.muted,
            ...
            <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:18,maxHeight:"calc(100vh - 340px)",overflowY:"auto"}}>
              ...
              {sorted.length===0
                ...
                :sorted.map((c,rank)=>{
                    ...
                    // Show improvement if has history
                    const bestTimeHistory=bestTimeStat?(c.customStatHistory||{})[bestTimeStat.id]:null;
                    const imp=bestTimeStat&&bestTimeHistory?getImprovement(bestTimeHistory,bestTimeStat):null;
                    return(
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          ...
                          <div style={{display:"flex",gap:5,alignItems:"center"}}>
                            ...
                            {bestTimeStat&&c.customStats?.[bestTimeStat.id]&&(
                              <div style={{color:C.accent,fontSize:10,fontWeight:700,fontFamily:"'Oswald',sans-serif"}}>{c.customStats[bestTimeStat.id]}</div>
                            )}
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12,maxHeight:"calc(100vh - 340px)",overflowY:"auto"}}>
                ...
                <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:18,display:"flex",alignItems:"center",gap:14}}>
                  <div style={{display:"flex",gap:6,flexShrink:0}}>
                    {(selCandObj.positions||[selCandObj.primaryPos||"CM"]).map((pos,i)=>(
                      <div key={pos} style={{width:i===0?52:36,height:i===0?52:36,borderRadius:i===0?12:8,
                        ...
                        fontFamily:"'Oswald',sans-serif",fontWeight:800,color:posColor(pos),
                        fontSize:i===0?18:13,opacity:i===0?1:.7}}>
                        {pos}
                      </div>
                    ))}
                  </div>
                  ...
                  <button onClick={()=>deleteCandidate(selCandObj.id)}
                    style={{background:"none",border:"none",color:C.muted,cursor:"pointer",padding:4}}><Trash2 size={15}/></button>
                </div>
                ...
                {/* ── MEASURABLES with history tracking ── */}
                {customStats.length>0&&(
                  <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:18}}>
                        ...
                        return(
                          <div key={stat.id} style={{background:C.surface,borderRadius:10,padding:"12px 14px",border:`1px solid ${C.border}`}}>
                            ...
                            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:history.length?10:8}}>
                              ...
                              <div style={{display:"flex",alignItems:"center",gap:8}}>
                                {imp&&(
                                  <span style={{color:imp.improved?"#66bb6a":C.danger,fontSize:12,fontWeight:700}}>
                                    {imp.label}
                                  </span>
                                )}
                                {bestVal&&(
                                  <span style={{color:C.accent,fontFamily:"'Oswald',sans-serif",fontWeight:900,fontSize:16}}>{bestVal}</span>
                                )}
                                {bestVal&&history.length>1&&(
                                  <span style={{color:C.muted,fontSize:9,fontWeight:700}}>★ best</span>
                                )}
                                {/* Note toggle */}
                                <button onClick={()=>setOpenStatNotes(p=>({...p,[nKey]:!noteOpen}))}
                                  style={{padding:"4px 7px",borderRadius:5,cursor:"pointer",fontSize:11,
                                    background:noteOpen||noteVal?C.accent+"22":"transparent",
                                    border:`1px solid ${noteOpen||noteVal?C.accent:C.border}`,
                                    color:noteOpen||noteVal?C.accent:C.muted}}>📝</button>
                              </div>
                            </div>
                            ...
                            {history.length>0&&(
                              <div style={{marginBottom:8,display:"flex",flexDirection:"column",gap:3}}>
                                {history.map((entry,i)=>{
                                  const isBest=entry.value===bestVal;
                                  const isLatest=i===history.length-1;
                                  return(
                                    <div key={entry.id} style={{display:"flex",alignItems:"center",gap:8,
                                      padding:"4px 8px",borderRadius:6,
                                      background:isBest?C.accent+"11":C.bg,
                                      border:`1px solid ${isBest?C.accent+"33":C.border}`}}>
                                      <span style={{color:C.muted,fontSize:10,minWidth:70}}>{entry.date}</span>
                                      <span style={{color:isBest?C.accent:C.text,fontWeight:isBest?700:400,
                                        fontFamily:"'Oswald',sans-serif",fontSize:14,flex:1}}>
                                        {entry.value}
                                      </span>
                                      ...
                                      <button onClick={()=>removeStatEntry(selCandObj.id,stat.id,entry.id)}
                                        style={{background:"none",border:"none",color:C.muted,cursor:"pointer",
                                          padding:"0 2px",fontSize:12,lineHeight:1}}>×</button>
                                    </div>
                                  );
                                })}
                              </div>
                            ...
                            <div style={{display:"flex",gap:6,alignItems:"center"}}>
                              {isTimeStat(stat)?(
                                <TimeInput value={entryVal}
                                  onChange={v=>setNewEntryVals(p=>({...p,[stat.id]:v}))}
                                  placeholder="0:00"
                                  style={iS({flex:1,padding:"6px 10px",fontSize:14})}/>
                              ):isSecondsStat(stat)?(
                                <input type="number" step="0.01"
                                  value={entryVal}
                                  onChange={e=>setNewEntryVals(p=>({...p,[stat.id]:e.target.value}))}
                                  placeholder="4.97"
                                  style={iS({flex:1,textAlign:"center",fontWeight:700,fontSize:14,padding:"6px 10px"})}/>
                              ):(
                              ...
                              <button
                                onClick={()=>addStatEntry(selCandObj.id,stat.id,entryVal)}
                                disabled={!entryVal?.toString().trim()}
                                style={{padding:"6px 14px",background:entryVal?.toString().trim()?C.accent:"#2a1000",
                                  border:"none",borderRadius:7,color:entryVal?.toString().trim()?"#000":C.muted,
                                  fontWeight:700,fontSize:12,cursor:entryVal?.toString().trim()?"pointer":"default",
                            ...
                            {noteOpen&&(
                              <div style={{marginTop:8}}>
                                <input value={noteVal}
                                  ...
                                  placeholder={`Note for ${stat.label}...`} autoFocus
                                  style={iS({fontSize:12,padding:"6px 10px"})}/>
                              </div>
                            )}
                            {!noteOpen&&noteVal&&(
                              <div onClick={()=>setOpenStatNotes(p=>({...p,[nKey]:true}))}
                                style={{marginTop:5,color:C.accent,fontSize:11,fontStyle:"italic",cursor:"pointer"}}>
                                📝 {noteVal}
                              </div>
                            )}
                          </div>
                      ...
                      return(
                        <div key={idx} onClick={()=>setPickingSlot({zone:zone.key,idx})}
                          style={{flex:"1 1 90px",minWidth:80,padding:"10px 6px",borderRadius:10,cursor:"pointer",
                            background:cand?"#1a0800":C.surface,border:`1.5px solid ${cand?zone.color+"66":C.border}`,
                          ...
                          {cand?<>
                            <div style={{width:34,height:34,borderRadius:8,background:pc+"22",border:`2px solid ${pc}44`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Oswald',sans-serif",fontWeight:900,color:pc,fontSize:15}}>
                              {(cand.positions||[cand.primaryPos||"CM"])[0]}
                            </div>
                            <div style={{color:C.text,fontSize:10,fontWeight:700,textAlign:"center",lineHeight:1.2}}>{cand.name.split(" ")[1]||cand.name.split(" ")[0]}</div>
                            ...
                            <div style={{width:34,height:34,borderRadius:8,background:C.border,display:"flex",alignItems:"center",justifyContent:"center",color:C.muted,fontSize:18}}>+</div>
                ...
                {allSorted.map(c=>{
                  ...
                  return(
                    <div key={c.id} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",
                      background:inLineup?C.accent+"11":C.surface,border:`1px solid ${inLineup?C.accent+"44":C.border}`,borderRadius:8,opacity:inLineup?.7:1}}>
      ...
      {activeTab==="stats"&&(
        <div style={{maxWidth:700}}>
          <div style={{color:C.muted,fontSize:13,marginBottom:18,lineHeight:1.6}}>
            Define measurable stats. Toggle type: <strong style={{color:C.accent}}>⏱ MM:SS</strong> for mile/drill times,
            <strong style={{color:"#42a5f5"}}> ⏱ Seconds</strong> for sprints like the 40-yard dash.
          </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr auto auto",gap:10,alignItems:"flex-end"}}>
              <div>
                ...
                <input value={newStatLabel} onChange={e=>setNewStatLabel(e.target.value)}
                  ...
                  placeholder="e.g. Mile Run, 40-Yard Dash" style={iS()}/>
              </div>
              <div>
          ...
          {customStats.length===0
            ...
            :<div style={{display:"flex",flexDirection:"column",gap:8}}>
                {customStats.map(stat=>{
                  const allEntries=tryout.candidates.flatMap(c=>(c.customStatHistory||{})[stat.id]||[]);
                  // Build per-candidate best values for leaderboard
                  const candBests=tryout.candidates
                    .map(c=>{
                  ...
                  return(
                    <div key={stat.id} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"14px 18px",display:"flex",alignItems:"flex-start",gap:14}}>
                      ...
                      <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4,flexShrink:0}}>
                        ...
                        {allEntries.length>0&&<div style={{color:C.muted,fontSize:11}}>{allEntries.length} total attempts</div>}
                      </div>
